...
## 10.3.1.1 Software Configurator
...
## 10.3.1.4 Config Delivery

### 10.3.1.4.1 Container mode (pro / OpenSVC / Kubernetes)
...
An **init container** (Alpine image) shares the network namespace with the database container and runs before the database starts.

**OpenSVC** uses a **bootstrap script** served by replication-manager. The init container downloads and executes it, passing credentials via OpenSVC secret and config services:

```ini
# OpenSVC container spec (generated by prov_opensvc_db.go)
[container#02]
type    = docker
image   = alpine
detach  = false
...
environment = REPLICATION_MANAGER_CLUSTER_NAME={namespace} REPLICATION_MANAGER_HOST_NAME={fqdn} REPLICATION_MANAGER_HOST_PORT={port}
```

The bootstrap script handles authentication, config tarball download, and extraction. Credentials are injected via OpenSVC secret services (encrypted) and config services — never hardcoded in the manifest.

**Kubernetes** currently uses a simpler direct fetch (no bootstrap script, no secrets):

```go
// Generated by prov_k8s_db.go
initContainers:
- name: db-init
  image: alpine
  command:
  - sh
  - -c
  - 'wget -qO- http://{monitorAddress}:{httpPort}/api/clusters/{cluster}/servers/{server}/{port}/config | tar xzvf - -C /data'
```

> **Note:** The K8s init container does not yet use HTTPS, bootstrap script, or K8s Secrets for credentials. See [issue #1497](https://github.com/signal18/replication-manager/issues/1497) for the convergence roadmap.
...
## 10.3.1.5 Starting the Database
...
### 10.3.1.5.3 Controlling Config Fetch
...
### 10.3.1.5.4 Preserving an Existing my.cnf
...
### 10.3.1.5.5 The `.system` Directory
...
### 10.3.1.5.6 nosplitpath — Flat Datadir Layout
...
See [Scripts Reference](../../01.orchestrators/05.scripts) for provisioning scripts, lifecycle hooks, and SSH overrides.

...
| `10001` | TCP | Proxy → repman | HTTP endpoint used by HAProxy and external check scripts to query master/slave status (`/clusters/{name}/servers/{host}/{port}/master-status`). Configured by `http-port`. |
...
| `3306` | TCP | DB → DB | MySQL/MariaDB replication stream (master → replica). |
...
## 2.7.5 Embedded Graphite Metrics
...
## 2.7.6 Multi-Node replication-manager (Arbitration)
...
## 2.7.8 Summary — Inbound Rules per Host
